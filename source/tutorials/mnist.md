...
shape of our data is `[64, 28, 28, 1]` (the batch is always the outer-most dimension).

*NOTE:** Recall that the `inputShape` in the config of our first `conv2d` did not specify the batch size (64). Configs arwritten to be batch-size-agnostic, to remain flexible and be able to accept batches of arbitrary size.
...
Every 5 steps, we construct `validationData`, an array of two elements containing a batch of MNIST images from the test set and their corresponding labels. We'll use this data to evaluate the accuracy of the model:
